...
class SlurmhelperCLI:
    ...
    def __init__(self):
        ...
        if args.cluster == 'midway2-scratch' and args.userid is None:
            raise ArgumentError("If you are using midway2-scratch, you must provide your user ID!")
        ...
        if not ((args.operation in {'list', 'init'}) or (args.operation == 'gen-scripts' and args.ids is None
            ...
            print("A total of {n} jobs would be affected by this call.".format(n=len(self.job_list)))
        ...
        assert set(self.job_list).issubset(self.__valid_ids), f"Some job ids provided are not in the scope of " \
        ...
        if 'base_directory_name' not in self.config.keys():
            print("Base working dir name not specified in your spec; using default ('working')")
