...
class Job:
    ...
    def __repr__(self):
        ...
        return "Job[{job:05d}{scripted}]".format(job=self.id,
                                                 scripted=scr_status)
    ...
    def _compute_specific_script(self, operation, script_template, verbose):
        ...
        fields = list(set([i[1] for i in Formatter().parse(script_template) if i[1] is not None]))
        if verbose:
            print("Template for %s requires %d unique parameters:  %s" % (
                    operation,
                    len(set(fields)),
        ...
        # Sanity check
        assert set(fmt_dict.keys()) == set(fields), "You're missing information!\n" \
                                                    "%d fields supplied: %s\n" \
                                                    "%d fields required: %s\n" % (len(fmt_dict),
                                                                                  ' '.join(fmt_dict.keys()),
                                                                                  len(set(fields)),
    ...
    def _write(self, operation):
        ...
        with open(p.joinpath(self._script_names[operation]), 'w') as writer:
            print("Writing job {id} {op} script to {path}".format(
                id = self.id,
                op = operation,
    ...
    def compute_paths(self, config=None, verbose=False):
        ...
        self._jd['this_job_run_script'] = str(Path(bd['job_scripts']).joinpath(self._script_names['run']))
        self._jd['this_job_copy_script'] = str(Path(bd['job_scripts']).joinpath(self._script_names['copy']))
        self._jd['this_job_clean_script'] = str(Path(bd['job_scripts']).joinpath(self._script_names['clean']))
        self._jd['this_job_log_file'] = str(Path(bd['job_logs']).joinpath("%s.txt" % (str(self))))
        self._jd['this_job_inputs_dir'] = str(Path(bd['job_inputs']).joinpath(str(self)))
        ...
        if config is not None:
            if 'output_path' in config.keys():
                ...
                if 'output_path_subject' in config.keys(): # requires output_path;
                    ...
                    subdir_fields = list(set([i[1] for i in Formatter().parse(subdir) if i[1] is not None]))
                    format_dict = {k:v for (k,v) in self.params.items() if k in subdir_fields}
                    self._jd['this_job_output_dir'] = os.path.join(self._jd['output_base_dir'],
                                                                   subdir.format(**format_dict))
                    if 'output_path_subject_expr' in config.keys(): # requires the above two!
                        re_template = config['output_path_subject_expr']
                        re_fields = list(set([i[1] for i in Formatter().parse(re_template) if i[1] is not None]))
                        format_dict = {k: v for (k, v) in self.params.items() if k in re_fields}
                        self._jd['this_job_output_expr'] = re_template.format(**format_dict)
                        self._jd['this_job_output_expr_fullpath'] = os.path.join(self._jd['this_job_output_dir'],
...
class TestableJob(Job):
    ...
    def __init__(self, db, paths, order_id, config):
        ...
        self._dir_inputs = os.path.join(paths['job_work'],"{job:05d}".format(job=self.id))
        self._dir_work = os.path.join(paths['job_inputs'],"{job:05d}".format(job=self.id))
        self._path_log = os.path.join(paths['job_logs'],"{job:05d}.txt".format(job=self.id))
        self.config = config
        # run tests
        self.run_tests()
    ...
    def get_logs(self):
        ...
        for test in self._tests_results:
            if not self._tests_results[test]['result']:
                logs.append("Test {test}: {log}".format(test=test, log=self._tests_results[test]['logs']))
    ...
    def _update_valid(self):
        results = [self._tests_results[test]['result'] for test in self._tests_results.keys()]
    ...
    def test_check_outputs(self):
        ...
        if not os.path.isdir(self._dir_outputs):
            ...
            if len(g_list) == self.config['expected_n_files']:
                ...
                rv['logs'].append('Number of files found does not match expectation;'
                                  ' expected = {exp:d}, found = {found:d}'.format(exp=self.config['expected_n_files'],
                                                                                  found=len(g_list)))
    ...
    def test_check_inputs(self):
        ...
        if not os.path.isdir(self._dir_inputs):
            ...
            rv['logs'].append("Input directory exists at {dir}".format(dir = self._dir_inputs))
    ...
    def test_check_work(self):
        ...
        if not os.path.isdir(self._dir_inputs):
            ...
            rv['logs'].append("Work directory exists at {dir}".format(dir = self._dir_work))
    ...
    def test_check_logs(self):
        ...
        if not os.path.isfile(self._path_log):
            rv[ 'logs' ].append("Job log file NOT found at {dir}".format(dir=self._path_log))
        else:
            with open(self._path_log,'r') as logfile:
                ...
                if len(log) == 0:
                    ...
                    rv['logs'].append("Log file is one line long? Weird. Line is:\n{line1}".format(line1=log[0]))
                elif log[-1] == "0" and log[-2] == 'SUCCESS':
                    rv[ 'result' ] = True
                else:
                    rv['logs'].append("Log ending is not as expected. Last two lines are:"
