...
def list_slurm(dirs):
    ...
    found_sbatch.sort()
    if not found_sbatch:
        raise Exception('No valid sbatch scripts found in your directory... whats up with that???')
    else:
        print("{num_sbatch} sbatch submission scripts found.".format(num_sbatch=len(found_sbatch)))
    ...
    if not found_arrays:
        ...
        for key in array_info.keys():
            print("ID: {sbatch_id}, array of length {array_length}. Includes jobs: ".format(sbatch_id=key,
                                                                                            array_length=len(array_info[key])))
